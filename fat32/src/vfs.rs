...
pub struct kstat {
    ...
    st_mode: u32,  // 文件的类型和存取的权限
    st_nlink: u32, // 连到该文件的硬连接数目，刚建立的文件值为1
    st_uid: u32,   // 文件所有者的用户ID
    st_gid: u32,   // 文件所有者的组ID
    st_rdev: u64,  // 若此文件为设备文件，则为其设备编号
    __pad: u64,    // 未使用
    st_size: i64,  // 文件字节数
    st_blksize: u32,   // 块大小
    __pad2: i32,       // 未使用
    st_blocks: u64,    // 文件所占块数
    st_atime_sec: i64,    // 最近一次访问时间秒
    st_atime_nsec: i64,   // 最近一次访问时间纳秒
    st_mtime_sec: i64,    // 最近一次修改时间秒
    st_mtime_nsec: i64,   // 最近一次修改时间纳秒
    st_ctime_sec: i64,    // 最近一次改变时间秒
    st_ctime_nsec: i64,   // 最近一次改变时间纳秒
    __unused: [u32; 2],   // 未使用
}
...
impl kstat {
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend_from_slice(&self.__pad.to_le_bytes());
        bytes.extend_from_slice(&self.st_size.to_le_bytes());
        bytes.extend_from_slice(&self.st_blksize.to_le_bytes());
        bytes.extend_from_slice(&self.__pad2.to_le_bytes());
        bytes.extend_from_slice(&self.st_blocks.to_le_bytes());
        bytes.extend_from_slice(&self.st_atime_sec.to_le_bytes());
        bytes.extend_from_slice(&self.st_atime_nsec.to_le_bytes());
        bytes.extend_from_slice(&self.st_mtime_sec.to_le_bytes());
        bytes.extend_from_slice(&self.st_mtime_nsec.to_le_bytes());
        bytes.extend_from_slice(&self.st_ctime_sec.to_le_bytes());
        bytes.extend_from_slice(&self.st_ctime_nsec.to_le_bytes());
        bytes.extend_from_slice(&self.__unused[0].to_le_bytes());
        bytes.extend_from_slice(&self.__unused[1].to_le_bytes());
...
pub struct dirent{
    d_ino: u64,   // 目录的inode节点号
    d_off: u64,   // 目录项的偏移
    d_reclen: u16,   // 这个目录项的长度
    d_type: u8,      // 目录类型
    d_name: [u8; 512],   // 目录名
}
...
impl dirent{
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        // 将每个字段转换为字节并写入字节向量
        bytes.extend_from_slice(&self.d_ino.to_le_bytes());
        bytes.extend_from_slice(&self.d_off.to_le_bytes());
        bytes.extend_from_slice(&self.d_reclen.to_le_bytes());
        bytes.extend_from_slice(&self.d_type.to_le_bytes());
        bytes.extend_from_slice(&self.d_name);
...
pub struct VFile {
    pub name: String,                      // 文件名
    pub short_sector: usize,               // 文件短目录项所在扇区
...
impl VFile {
    ...
    // 长目录名来寻找目录
    fn find_long_name(&self, name: &str, dir_ent: &ShortDirEntry) -> Option<Arc<VFile>> {
    ...
    /// 查找短文件名目录
    fn find_short_name(&self, name: &str, dir_ent: &ShortDirEntry) -> Option<Arc<VFile>> {
    ...
    /// 根据名称搜索当前目录下的文件
    pub fn find_vfile_byname(&self, name: &str) -> Option<Arc<VFile>> {
        assert!(self.is_dir());
        // 将文件名和扩展分开
    ...
    pub fn find_vfile_bypath(&self, path: Vec<&str>) -> Option<Arc<VFile>> {
        ...
        let mut current_vfile = Arc::new(self.clone());
        for i in 0..len {
            if path[i] == "" || path[i] == "." {
                // 跳过，表示仍然为当前目录
                continue;
    ...
    pub fn dirent_info(&self) -> Option<dirent> {
        self.read_short_dirent(|sde: &ShortDirEntry| {
            ...
            bytes[..self.name.as_bytes().len()].clone_from_slice(self.name.as_bytes());            
            return Some(dirent{
                d_ino: first_clu as u64,
                d_off: self.short_offset as u64,
                d_reclen: DIRENT_SZ as u16,
                d_type: sde.attribute(),
                d_name: bytes,
            });
        }
        )   
